...
//        // Common listener for all WARS/JSPs running under Jetty to provide a bridge back to here.
//        ServletContextHandler commonContext = new ServletContextHandler();
//		commonContext.setContextPath("/controller");
//		commonContext.addEventListener(new HomeAutomationContextListener(controller));
//		commonContext.setClassLoader(Thread.currentThread().getContextClassLoader()); //THIS LINE IS THE KEY
//		contextCollection.addHandler(commonContext);
        ...
        contextAttributeCustomizer.setAttribute(HomeAutomationContextListener.INTERFACE_CONTROLLER, 
        		new HomeAutomationContextListener(controller));
	...
	public static class ContextAttributeCustomizer implements AppLifeCycle.Binding
    {
        ...
        public void processBinding(Node node, App app) throws Exception
        {
            ...
            ClassLoader origLoader = app.getClass().getClassLoader();
            URL[] origURLs = ((URLClassLoader) origLoader).getURLs();
            
            URL[] addURLs = ((URLClassLoader) classLoader).getURLs();
            
            URL[] combinedURLs = new URL[origURLs.length+addURLs.length];
            
            for (int i = 0; i < origURLs.length; i++) {
				combinedURLs[i]=origURLs[i];
				System.out.println("O:"+origURLs[i].getPath());
			}
            for (int i = 0; i < addURLs.length; i++) {
				combinedURLs[i+origURLs.length]=addURLs[i];
				System.out.println("A:"+addURLs[i].getPath());
			}
            
//            handler.setClassLoader(new URLClassLoader(combinedURLs, origLoader));
